...
public class SponeManager : MonoBehaviour
{
    ...
    public static List<GameObject> list_Prefabs; // 프리팹 리스트
    public static List<Transform> list_ItemSpawnPoints; // 아이템 스폰 포인트 리스트
    public static List<Transform> list_MosterSpawnPoints; // 몬스터 스폰 포인트 리스트
    //public static List<Transform> list_PlayerSpawnPoints; // 플레이어 스폰 포인트 리스트
    
    public static void m_CreatePlayer()
    {
        if (UserData.gender == 0)
        {
            obj_local = PhotonNetwork.Instantiate(list_Prefabs[UserData.type].name, spawn_point.position, Quaternion.identity); // 남성 플레이어 프리팹 생성
        }
        else if (UserData.gender == 1)
        {
            obj_local = PhotonNetwork.Instantiate(list_Prefabs[UserData.type].name, spawn_point.position, Quaternion.identity); // 여성 플레이어 프리팹 생성
        }
    }
